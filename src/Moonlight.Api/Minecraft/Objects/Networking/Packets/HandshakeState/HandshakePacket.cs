...
namespace Moonlight.Api.Minecraft.Objects.Networking.Packets.HandshakeState
{
    public record HandshakePacket : AbstractPacket
    {
        /// <inheritdoc />
        ...
        /// <summary>
        /// The server address that the client is attempting to connect to.
        /// </summary>
        public IPAddress ServerAddress { get; private set; } = IPAddress.Any;

        /// <summary>
        /// The server port that the client is attempting to connect to.
        /// </summary>
        public ushort ServerPort { get; private set; } = 25565;

        /// <summary>
        /// The next state that the client will be in.
        /// </summary>
        ...
        /// <inheritdoc />
        public HandshakePacket(int id = 0x01, params byte[] data) : base(id, data) { }

        /// <summary>
        /// Creates a new HandshakePacket.
        /// </summary>
        /// <param name="protocolVersion">The Minecraft protocol version that's being used.</param>
        /// <param name="serverAddress">The server address that's being connected to.</param>
        /// <param name="serverPort">The server port that's being connected to.</param>
        /// <param name="nextClientState">The next state that the client will be in.</param>
        ...
        /// <summary>
        /// Reads the first (and subsequently second and third bytes, if required) of the incoming server packet. Constructs a HandshakePacket from any Minecraft version.
        /// </summary>
        ...
        {
            byte packetLengthOrId = packetHandler.ReadByte();
            switch (packetLengthOrId)
            {
                case 0xFE: // Beta 1.8-1.6 server status packet
                    ...
                    {
                        packetLength = packetHandler.ReadByte();
                    }
                    ...
                    else
                    {
                        ...
                        {
                            pluginPacketIdentifier = packetHandler.ReadByte();
                        }
                        ...
                        else // 1.6 to pre-1.7. By this point, we have read three bytes: [0xFE, 0x01, 0xFA]
                        {
                            ...
                            tempPacketHandler.WriteByte(0xFE);
                            tempPacketHandler.WriteByte((byte)packetLength);
                            tempPacketHandler.WriteByte((byte)pluginPacketIdentifier);
