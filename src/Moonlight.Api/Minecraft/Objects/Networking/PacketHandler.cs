...
using Moonlight.Api.Minecraft.Objects.Networking.Packets;
...
namespace Moonlight.Api.Minecraft.Objects.Networking
{
    public class PacketHandler : IPacketHandler
    {
        ...
        /// <summary>
        /// Creates a new PacketHandler for use with a stream.
        /// </summary>
        /// <param name="stream">The stream to read from. Should follow Minecraft protocol.</param>
        /// <param name="cancellationToken">A cancellation token to be used with the stream's async methods.</param>
        ...
        #region ReadMethods
        /// <inheritdoc />
        public virtual byte ReadByte() => (byte)Stream.ReadByte();

        /// <inheritdoc />
        public virtual sbyte ReadSignedByte() => (sbyte)Stream.ReadByte();
        ...
        /// <inheritdoc />
        public AbstractPacket ReadPacket(int packetLength = -1)
        {
        ...
        /// <inheritdoc />
        public T ReadPacket<T>(int packetLength = -1) where T : AbstractPacket, new()
        {
            if (Stream.CompressionEnabled)
            {
                PacketHandler uncompressedPacketHandler = new(Stream.BaseStream, CancellationToken);
        ...
        private AbstractPacket ReadUncompressedPacket(int length = -1)
        {
            if (length == -1)
            {
                length = ReadVarInt();
        ...
        private T ReadUncompressedPacket<T>(int packetLength = -1) where T : AbstractPacket, new()
        {
            if (packetLength == -1)
            {
        ...
        /// <inheritdoc />
        public virtual async Task<AbstractPacket> ReadPacketAsync(int packetLength = -1)
        {
            if (Stream.CompressionEnabled)
            {
                PacketHandler uncompressedPacketHandler = new(Stream.BaseStream, CancellationToken);
        ...
        /// <inheritdoc />
        public virtual async Task<T> ReadPacketAsync<T>(int packetLength = -1) where T : AbstractPacket, new()
        {
            if (Stream.CompressionEnabled)
            {
                PacketHandler uncompressedPacketHandler = new(Stream.BaseStream, CancellationToken);
        ...
        private async Task<AbstractPacket> ReadUncompressedPacketAsync(int length = -1)
        {
            if (length == -1)
            {
                length = ReadVarInt();
        ...
        private async Task<T> ReadUncompressedPacketAsync<T>(int length = -1) where T : AbstractPacket, new()
        {
            if (length == -1)
            {
                length = ReadVarInt();
        ...
        #region WriteMethods
        /// <inheritdoc />
        public virtual void WriteByte(byte value) => Stream.WriteByte(value);
        /// <inheritdoc />
        public virtual void WriteSignedByte(sbyte value) => Stream.WriteByte((byte)value);
        /// <inheritdoc />
        ...
        /// <inheritdoc />
        public virtual void WriteByteArray(byte[] value) => Stream.Write(value);
        ...
        /// <inheritdoc />
        public virtual void WritePacket(AbstractPacket value)
        {
        ...
        /// <inheritdoc />
        public virtual async Task WriteByteArrayAsync(byte[] value) => await Stream.WriteAsync(value, CancellationToken);
        ...
        /// <inheritdoc />
        public virtual async Task WritePacketAsync(AbstractPacket value)
        {
            ArgumentNullException.ThrowIfNull(value, nameof(value));
