...
namespace Moonlight.Api.Minecraft.Objects.Chat
{
    public class ChatComponent : IChatComponent
    {
        ...
        public bool HasFormatting() => (Bold.HasValue && Bold.Value) || (Italic.HasValue && Italic.Value) || (Underlined.HasValue && Underlined.Value) || (Strikethrough.HasValue && Strikethrough.Value) || (Obfuscated.HasValue && Obfuscated.Value) || string.IsNullOrEmpty(Insertation?.Trim()) || Extra?.Length != 0 || string.IsNullOrEmpty(Font?.Trim());
        public void SetFont(FontType type) => Font = "minecraft:" + Enum.GetName(type)?.ToLowerInvariant() ?? "default";
        public static IChatComponent Create(string text) => new ChatComponent(text);
        ...
        private string ConvertPropertiesToSectionSigns()
        {
            ...
            if (Obfuscated.HasValue && Obfuscated.Value)
            {
            ...
            if (Underlined.HasValue && Underlined.Value)
            {
