...
using Moonlight.Api.Minecraft.Objects.Networking.Packets.HandshakeState;
...
namespace Moonlight.Server.Networking
{
    public class ServerListener
    {
        ...
        public ServerListener(ILogger logger, IConfiguration configuration)
        {
        ...
        internal async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (!IPAddress.TryParse(Configuration.GetValue("server:ip", "0.0.0.0"), out IPAddress? listeningAddress) && listeningAddress != IPAddress.Any)
            {
                try
                {
                    listeningAddress = Dns.GetHostEntry(Configuration.GetValue("server:ip", "0.0.0.0")).AddressList[0];
                }
                catch (Exception exception)
                {
                    Logger.Fatal("Ip Address {ipAddress} is not valid. Error message:\n\t{errorMessage}", Configuration.GetValue("server:ip", "0.0.0.0"), exception.Message);
            ...
            TcpListener tcpListener = new(listeningAddress, Configuration.GetValue("server:port", 25565));
            tcpListener.Start(Configuration.GetValue("server:max_pending_connections", 100));
            Logger.Information("Server listening on {address}, port {port}", Configuration.GetValue("server:ip", "0.0.0.0"), Configuration.GetValue("server:port", 25565));
                ...
                switch (handshakePacket.NextClientState)
                {
                    case ClientState.Status:
                        ...
                        else
                        {
                            ...
                            AbstractPacket requestPacket = await packetHandler.ReadPacketAsync();
                            ...
                            ResponsePacket responsePacket = new(serverStatus);
                    ...
                    case ClientState.Login:
                        ...
                        if (handshakePacket.ProtocolVersion == -1)
                        {
                            byte[] data = Encoding.BigEndianUnicode.GetBytes(Configuration.GetValue("server:invalid_protocol_message", "You must be on version 1.17.1!"));
                            packetHandler.WriteByte(0xFF); // Packet Id
                        ...
                        {
                            await new MinecraftClient(tcpClient.GetStream(), false, Log.ForContext<MinecraftClient>(), Configuration, cancellationToken).Login();
                        }
                        else
                        {
                            packetHandler.WritePacket(new DisconnectPacket(Configuration.GetValue("server:invalid_protocol_message", "&cYou must be on version 1.17.1!")));
                            Logger.Verbose("{ipAddress} is using an unsupported protocol version: {protocolVersion}. Disconnect Packet Sent,\n\tReason: {invalidProtocolMessage}", (tcpClient.Client.RemoteEndPoint as IPEndPoint)?.Address.ToString() ?? "An unknown ip", handshakePacket.ProtocolVersion, Configuration.GetValue("server:invalid_protocol_message", "You must be on version 1.17.1!"));
                            packetHandler.Dispose();
                            tcpClient.Dispose();
                        }
                        break;
