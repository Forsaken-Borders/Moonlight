...
using Moonlight.Api.Networking.Packets.HandshakeState;
using Moonlight.Api.Networking.Packets.LoginState;
using Moonlight.Api.Networking.Packets.StatusState;
using Moonlight.Api.Types.Chat;
using Moonlight.Api.Types.ServerPing;
using Moonlight.Network.Packets.StatusState;
using Serilog;
                ...
                switch (handshakePacket.NextClientState)
                {
                    case ClientState.Status:
                        ...
                        else
                        {
                            ...
                            Packet requestPacket = await packetHandler.ReadPacketAsync();
                    ...
                    case ClientState.Login:
                        ...
                        {
                            await new MinecraftClient(tcpClient, Log.ForContext<MinecraftClient>(), Program.Configuration, cancellationToken).Login();
                        }
                        else
                        {
                            packetHandler.WritePacket(new DisconnectPacket(Program.Configuration.GetValue("server:invalid_protocol_message", "&cYou must be on version 1.17.1!")));
